...
import views.ClientWithdrawCellFactory;

public class WithdrawController implements Initializable{

	@FXML
	private Button btnsearch,btnwithd;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		btnwithd.setOnAction(event -> onWithdraw(em));
		
	}
	 
	private void onWithdraw(EntityManager em) {
		try {
		...
		if(txtsearch.getText().isEmpty()) {
			showAlert(AlertType.ERROR, "SEARCH FIELD CAN'T BE EMPTY", "PLEASE ENTER PAYEEADDRESS", null);
			return;
		}
		...
		if(choice.getValue().toString().equals("savings")) {
			Model.getInstance().deductSenderBalance(em, txtsearch.getText(), new BigDecimal(txtamount.getText()), SavingsAccount.class);
		}
		
		if(choice.getValue().toString().equals("checking")) {
			Model.getInstance().deductSenderBalance(em, txtsearch.getText(), new BigDecimal(txtamount.getText()), CheckingAccount.class);
	...
	public void setcellfactory() {
		resultLV.setCellFactory(new Callback<ListView<ClientView>, ListCell<ClientView>>() {
            ...
            public ListCell<ClientView> call(ListView<ClientView> listView) {
            	...
            	return new ClientWithdrawCellFactory();
            }
        });
